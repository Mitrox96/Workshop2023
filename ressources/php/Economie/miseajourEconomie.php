...
if (!empty($data)) {
    try {
        // Loop through the data and compare/update each entry in the "Economie" table
        foreach ($data as $entry) {
            $id = $entry['id'];

            // Check if the entry exists in the database
            $checkQuery = "SELECT * FROM Economie WHERE id = :id";
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $updateQuery = "UPDATE Economie SET " . implode(', ', $updateColumns) . " WHERE id = :id";
                ...
                $insertQuery = "INSERT INTO Economie (id, nom, prenom, ville, cursus)";
