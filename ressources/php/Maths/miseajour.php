...
if (!empty($data)) {
    try {
        // Loop through the data and compare/update each entry in the "Maths" table
        foreach ($data as $entry) {
            $id = $entry['id'];

            // Check if the entry exists in the database
            $checkQuery = "SELECT * FROM Maths WHERE id = :id";
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $updateQuery = "UPDATE Maths SET " . implode(', ', $updateColumns) . " WHERE id = :id";
                ...
                $insertQuery = "INSERT INTO Maths (id, nom, prenom, ville, cursus)";
