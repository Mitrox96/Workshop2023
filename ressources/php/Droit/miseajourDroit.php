...
if (!empty($data)) {
    try {
        // Loop through the data and compare/update each entry in the "Droit" table
        foreach ($data as $entry) {
            $id = $entry['id'];

            // Check if the entry exists in the database
            $checkQuery = "SELECT * FROM Droit WHERE id = :id";
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $updateQuery = "UPDATE Droit SET " . implode(', ', $updateColumns) . " WHERE id = :id";
                ...
                $insertQuery = "INSERT INTO Droit (id, nom, prenom, ville, cursus)";
